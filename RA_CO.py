...
<p style="margin: 10px 0 0 0; color: #6c757d; font-size: 18px;">Hướng dẫn: Bên trái upload file exel tải từ báo cáo Danh sách tờ khai trên phần mềm SLXNK; Bên phải là file từ Báo cáo danh sách chi tiết hàng tờ khai; Lưu ý: File exel tải về bắt buộc xoá 8 dòng đầu tiên để công cụ nhận diện đúng tiêu đề các cột.</p>
</div>
""",
...
with col1:
    ...
    if uploaded_file_bnn is not None:
        if st.button("Kiểm tra dữ liệu C/O - BNN", key="btn_bnn", type="primary"):
            with st.spinner("Đang xử lý dữ liệu CO-BNN..."):
                try:
                    ...
                    st.session_state["res_bnn"] = final_df
                except Exception as e:
                    st.error(f"[LỖI XỬ LÝ CO-BNN]: {str(e)}")

        if "res_bnn" in st.session_state and st.session_state["res_bnn"] is not None:
            final_df = st.session_state["res_bnn"]
...
with col2:
    ...
    if uploaded_file_hd is not None:
        if st.button("Kiểm tra dữ liệu Hoá Đơn", key="btn_hd", type="primary"):
            with st.spinner("Đang xử lý dữ liệu Hoá đơn..."):
                try:
                    ...
                    st.session_state["res_hd"] = final_hd_df
                except Exception as e:
                    st.error(f"[LỖI XỬ LÝ HOÁ ĐƠN]: {str(e)}")

        if "res_hd" in st.session_state and st.session_state["res_hd"] is not None:
            final_hd_df = st.session_state["res_hd"]
        ...
        st.session_state["res_hd"] = None

st.markdown("---")
st.markdown(
    "<p style='text-align: center; color: #6c757d; font-size: 25px;'>< Bản quyền thuộc về: HỒ LÂM TÙNG - 0988 767413 - CHI CỤC HẢI QUAN KHU VỰC VII</b></p>",
    unsafe_allow_html=True,
)
